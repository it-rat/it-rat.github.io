...
**On premises.** On 2026-09-17 the whole stack went onto a small Debian box behind a home router, carrier-grade NAT and all, with its gateway published only on a tunnel address. Two clusters standing in for a customer's, one in AWS and one in GCP, joined the tunnel and ran their agents through the box to the real Anthropic API: seventeen calls each, a 402 on the eighteenth, both inside one minute on one event log. The box told the clouds apart by key and by identity, froze one of them with a single policy while the other kept running, and then took sixteen injected failures: a killed node, a cut path, a stopped policy plane, a stopped store, a blocked provider, a loop, a storm, a reboot under load, a tampered evidence pack. Eight held as designed and eight found something; eleven of the run's sixteen findings were fixed on main within a day, and none of the fixes is in a released image yet, which [what is proven](https://it-rat.com/what-is-proven.html) says tile by tile. Both cloud accounts read back empty afterwards. The day cost USD 0.18.
...
Every service has its own room: what it does, how it behaves over a live time window, and where it sits in the wiring. Every room is Apache-2.0, with nothing behind a licence. Genaryx, first on the rail, is the control room, and it runs on your own infrastructure like the rest. After it come the planes themselves, in the order a fleet tends to adopt them: money and the crew that reads the bill, then policy, access, egress and delegation, then the planes that watch rather than decide. Platform, last, is the contract they all share. Walk it with the arrows on each page, scroll this rail sideways, or hit `⌘K` and type two letters.
...
Every request carries an Agent Passport. An agent has two ways out, not one: a call to a model, and a fetch of something on the internet, and each has a gate standing in it. Above them sit the two questions a gate asks, and they are asked differently. Policy answers per action. Delegation deliberately does not: it publishes keys and a revocation list that enforcement points poll, and it refuses to offer introspection precisely so that it never becomes a service every gate has to reach on every call. Below, memory, identity, crypto, quality, rehearsal and alerting read the same NDJSON stream off-path. Nothing here is a dashboard after the fact: the gates enforce in-line, in seconds. The drawing stops at the planes a request actually touches, so three that matter are named on the rail rather than here: Trailryx seals the bus into a record, Pocket holds the switch on a device this box never touches, and CostCrew's crew are agents the same gates govern like any other.

New to this? [Seven guides](https://it-rat.com/guides.html) explain the field rather than the products: [agent governance](https://it-rat.com/ai-agent-governance.html), [FinOps for AI](https://it-rat.com/finops-for-ai.html), [agent security](https://it-rat.com/ai-agent-security.html), [observability versus governance](https://it-rat.com/ai-observability-vs-governance.html), [MCP security](https://it-rat.com/mcp-security.html), [agent identity](https://it-rat.com/agent-identity.html), and [the shapes of the tooling itself](https://it-rat.com/agent-tooling-compared.html). Four more cover this stack in practice: [a first alert](https://it-rat.com/first-alert.html), [one incident end to end](https://it-rat.com/one-incident-end-to-end.html), [what runs where and what it costs](https://it-rat.com/what-runs-where.html), and [what is proven and what is not](https://it-rat.com/what-is-proven.html).
