...
Every request carries an Agent Passport. Money and policy sit in the request path; memory, identity, crypto, quality, rehearsal and alerting watch the same NDJSON event stream off-path. Nothing here is a dashboard after the fact: the gateway enforces in-line, in seconds. The drawing is deliberately narrower than the corridor: it is the spine, one call from an agent to a provider and the bus that call feeds, so the planes that join it elsewhere are named on the rail rather than drawn here. Egress and delegation decide before a fetch and before a token is honoured, Trailryx seals the bus into a record, Pocket holds the switch on a device this box never touches, and CostCrew's crew are agents the same spine governs like any other.
